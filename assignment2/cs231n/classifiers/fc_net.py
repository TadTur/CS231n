...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache2 = affine_relu_forward(scores1, W2, b2)
        ...
        dx, grads['W2'], grads['b2'] = affine_relu_backward(soft_grad, cache2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            x, cache = affine_relu_norm_forward(x, w, b,gamma, beta, bn_params, self.normalization, self.use_dropout, self.dropout_param)
            caches.append(cache)
        ...
        for i in range(self.num_layers - 2, -1, -1):
            dx, dw, db, dgamma, dbeta = affine_relu_norm_backward(dout, caches[i], self.normalization, self.use_dropout)
...
def affine_relu_norm_forward(x, w, b, gamma, beta, bn_param, normalization, use_dropout, dropout_param):
    ...
    cache = (fc_cache, relu_cache, norm_cache, do_cache)
    return out, cache


def affine_relu_norm_backward(dout, cache, normalization, use_dropout):
    fc_cache, relu_cache, norm_cache, do_cache = cache
    if use_dropout:
